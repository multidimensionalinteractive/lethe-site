...
document.querySelectorAll(".section, .works-head, .archive-head, .work, .drawer, .correspondence-panel, .veracity").forEach((section) => {
...
function ensureImageLightbox() {
    ...
    imageLightbox.setAttribute("aria-label", "Archive object preview");
    imageLightbox.setAttribute("aria-hidden", "true");
...
function openImageLightbox(image) {
    ...
    const caption = figure?.querySelector("figcaption")?.textContent?.trim() || image.alt || "Archive object";
...
lightboxImages.forEach((image) => {
    ...
    image.setAttribute("aria-label", `Open archive preview: ${image.alt || "selected object"}`);
...
function applyScrollFade(target, progress, transform, settleDelay = 420) {
    ...
    target.style.opacity = String(Math.max(0.45, 1 - progress * 0.72));
